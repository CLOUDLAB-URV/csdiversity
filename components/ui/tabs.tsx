...
const TabsList = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(({ className, ...props }, ref) => (
  <div
    ...
    className={cn(
      ...
      "backdrop-blur supports-[backdrop-filter]:bg-muted/80 border border-slate-200 dark:border-slate-800",
      className
    )}
    {...props}
  />
...
const TabsTrigger = React.forwardRef<HTMLButtonElement, TabsTriggerProps>(({ value, className, children, ...props }, ref) => {
  ...
  return (
    <button
      ...
      type="button"
      className={cn(
        "inline-flex min-w-[140px] items-center justify-center whitespace-nowrap rounded-lg px-4 py-2 text-sm font-medium transition-all",
