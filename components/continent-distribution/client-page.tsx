...
const GeoModeToggle = ({ geoMode, onChange }: GeoModeToggleProps) => (
...
export function ClientContinentDistributionPage({
  initialData,
  conferences,
  years,
  papersCountryRaw,
}: ClientContinentDistributionPageProps) {
  ...
  const handleYearChange = useCallback((year: number | undefined) => {
    ...
    trackEvent({
      action: "continent_year_change",
      category: "continent_distribution",
  ...
  const handleConferencesChange = useCallback((list: string[]) => {
    ...
    trackEvent({
      action: "continent_conference_filter",
      category: "continent_distribution",
      value: list.length,
    });
  }, []);
  ...
  const handleGeoModeChange = useCallback((mode: 'continent' | 'country') => {
    setGeoMode(mode);
    trackEvent({
      action: "continent_geo_mode_change",
      category: "continent_distribution",
      label: mode,
      params: {
  ...
  const filteredCountryRows = useMemo(() => {
    ...
    return papersCountryRaw.filter((row) => {
  ...
  }, [papersCountryRaw, selectedConferences, selectedYear]);

  const countryChart = useMemo(() => {
    if (!filteredCountryRows || filteredCountryRows.length === 0) {
      return {
                ...
                <div className="space-y-1">
                  ...
                  <CardDescription>
                    {geoMode === 'continent'
                      ...
                      : (isSingleConferenceView
                          ? "Top countries share of accepted papers by year for the selected conference."
                          : "Top countries share of accepted papers across selected conferences.")}
                  </CardDescription>
                </div>
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    {geoMode === 'continent' ? (
                      <BarChart
                        ...
                        role="img"
                        aria-label={`Accepted papers distribution by continent ${selectedConferences.length > 0 ? 'for selected conferences' : selectedYear ? `for ${selectedYear}` : ''}`}
                      >
                      ...
                      <BarChart
                        ...
                        margin={{ top: 20, right: 30, left: 20, bottom: 60 }}
                        role="img"
                        aria-label={`Accepted papers distribution by country ${selectedConferences.length > 0 ? 'for selected conferences' : selectedYear ? `for ${selectedYear}` : ''}`}
                      >
                        <CartesianGrid strokeDasharray="3 3" stroke="#e5e7eb" opacity={0.3} />
                        <XAxis
                          dataKey="conference"
